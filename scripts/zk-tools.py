...
## Count deleting node that are older than 15 min
# len(filter_by_min_age(60 * 15)(filter_by_state("deleting")(get_nodes(zk))))
...
def filter_by_state(name):
    def _func(nodes):
        return list(filter(lambda x: x[1]['state'] == name, nodes))
    return _func

def filter_by_min_age(age):
    """ age is in second """
    def _func(nodes):
        return list(filter(lambda x: time.time() - x[1]['state_time'] >= age, nodes))
    return _func
